      ...
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
          <div class="btn-group me-2">
            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="print()">Print Halaman</button>
          </div>
        </div>
      </div>
        ...
        <table class="table table-striped table-sm">
          <thead>
            <tr>
              ...
              <th scope="col">Total Orderan</th>
              <th scope="col">No. HP</th>
              <th scope="col">Order Bulan Ini</th>
            </tr>
          </thead>
          <tbody>
            <?php
              $query = "SELECT DISTINCT nama FROM orderan;";
              $resultNama = mysqli_query($koneksi, $query);
              while ($nama = mysqli_fetch_assoc($resultNama)) {
                // TOTAL ORDER
                $strNama = $nama["nama"];
                $query = "SELECT COUNT(*) as jml FROM orderan WHERE nama = '$strNama';";
                $resultTotal = mysqli_query($koneksi, $query);
                $totalOrder = mysqli_fetch_assoc($resultTotal);
                // NO HP
                $query = "SELECT `no hp` FROM orderan WHERE nama = '$strNama';";
                $resultHp = mysqli_query($koneksi, $query);
                $nohp = mysqli_fetch_assoc($resultHp);
                // ORDER BULAN
                $query = "SELECT COUNT(*) as jml FROM orderan WHERE nama = '$strNama' AND MONTH(waktu) = MONTH(NOW());"; 
                $resultBulan = mysqli_query($koneksi, $query);
                $bulanOrder = mysqli_fetch_assoc($resultBulan);
             ?>
            <tr>
              <td><?php echo $nomor ?></td>
              <td><?php echo $nama["nama"] ?></td>
              <td><?php echo $totalOrder["jml"] ?></td>
              <td><?php echo $nohp["no hp"] ?></td>
              <td><?php echo $bulanOrder["jml"] ?></td>
            </tr>
            <?php
          $nomor += 1;
          };
